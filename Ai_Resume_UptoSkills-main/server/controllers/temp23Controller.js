...
const GeminiFunctionField = async (
  category,
  fieldName,
  userInput,
  retries = 3
...
Return only valid JSON in this format: {"${fieldName}": "Enhanced ATS-optimized text"}
      ...
      if (!responseText) {
        console.error(
          `❌ Empty AI response for ${category} field '${fieldName}'`
        );
        return userInput;
      }
      ...
      if (
        parsedResponse &&
        parsedResponse[fieldName] &&
        parsedResponse[fieldName].trim().length > 0
      ) {
        return parsedResponse[fieldName];
      } else {
        console.warn(
          `❌ Invalid AI response for ${category} field '${fieldName}'`
        );
        return userInput;
      }
    } catch (error) {
      console.error(
        `❌ Error enhancing ${category} field '${fieldName}':`,
        error.message
      );
      retries--;
  ...
  console.error(
    `❌ AI failed for ${category} field '${fieldName}' after retries`
  );
  return userInput;
};
...
const enhanceExperience = async (experienceArray) => {
  ...
  for (const exp of experienceArray) {
    const enhancedDescription = await GeminiFunctionField(
      "experience",
      "description",
      exp.description || ""
    );
...
const enhanceAchievements = async (achievementsArray) => {
  ...
  for (const ach of achievementsArray) {
    const enhancedDescribe = await GeminiFunctionField(
      "achievements",
      "describe",
      ach.describe || ""
    );
...
const createResume = async (req, res) => {
  try {
    ...
    if (existingResume) {
      return res
        .status(409)
        .json({ message: "Resume already exists", data: existingResume });
    }
    ...
    res
      .status(201)
      .json({ message: "Resume created successfully", data: savedResume });
  } catch (error) {
...
const saveResume = async (req, res) => {
  try {
    ...
    console.log(
      "🔹 Received Resume Data for Saving:",
    ...
    const formattedResume = {
      ...
      achievements: Array.isArray(resumeData.achievements)
        ...
        : [],
      courses: Array.isArray(resumeData.courses)
...
const enhanceField = async (req, res) => {
  try {
    ...
    if (!resumeId || !field) {
      return res
        .status(400)
        .json({ message: "Resume ID and field are required" });
    }
    ...
    switch (field) {
      ...
      default:
        prompt = `Enhance this ${field} content to be more professional and ATS-friendly: ${JSON.stringify(
          content
        )}`;
    }
    ...
    try {
      ...
      return res
        .status(500)
        .json({ message: "Failed to process AI enhancement" });
    }
    ...
    if (field === "experience") {
      const enhancedExperience = {
        ...
        accomplishment:
          enhancedContent.accomplishment || content.accomplishment,
      };
      return res.json({
    ...
    res
      .status(500)
      .json({ message: "Error enhancing field", error: error.message });
  }
};
...
const generateHTML = (resumeData) => {
  return `
    ...
    <body>
      ...
      ${
        resumeData?.summary
          ? `<div class="section"><h2>Summary</h2><p>${resumeData.summary}</p></div>`
          : ""
      }
      ${
        resumeData?.experience?.length
          ? `<div class="section"><h2>Experience</h2>${resumeData.experience
              .map(
                (exp) => `
      ...
      ${
        resumeData?.skills?.length
          ? `<div class="section"><h2>Skills</h2><ul>${resumeData.skills
      ...
      ${
        resumeData?.courses?.length
          ? `<div class="section"><h2>Courses</h2>${resumeData.courses
              .map(
                (course) => `
      ...
      ${
        resumeData?.projects?.length
          ? `<div class="section"><h2>Projects</h2>${resumeData.projects
              .map(
                (project) => `
